...
public class ShowPreventivoController implements Initializable {
    ...
    public void controlloArrivo(ActionEvent event) {
        //LocalDate date = LocalDate.now();
        LocalDate date = LocalDate.parse("2024-06-19");
        if(date.toString().equals(dataRit.getText()))
        {
            DButils.getIdFromImpiegati(event, Richiedente.getText());
            testAutoData.arrivata = "true";
            DButils.setArrivo(Richiedente.getText(),testAutoData.arrivata,testAutoData.idUserFromImpiegati);
            DButils.eliminaPrev(testAutoData.codPreventivo);
        }
    }
}
